...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, BatchNormalization, LeakyReLU, Reshape, Conv2DTranspose, Activation, ReLU, Layer
...
class Generator_Model(tf.keras.Model):
    def __init__(self):
        ...
        self.net.add(BatchNormalization())
        self.net.add(LeakyReLU(alpha=.02))
        self.net.add(Conv2DTranspose(64, (5,5), strides=(2,2), padding='same', kernel_initializer = initializer))
        self.net.add(BatchNormalization())
        self.net.add(LeakyReLU(alpha=.02))
...
class Discriminator_Model(tf.keras.Model):
    def __init__(self):
        ...
        initializer = tf.random_normal_initializer(stddev=0.2)
        self.net = tf.keras.Sequential()
        self.net.add(Conv2D(64, (5,5), strides=(2,2), padding = 'same', input_shape=[64,64,3], kernel_initializer = initializer))
        self.net.add(LeakyReLU(alpha=0.02))
        self.net.add(Conv2D(128, (5,5), strides=(2,2), padding = 'same', kernel_initializer = initializer))
        self.net.add(BatchNormalization())
        self.net.add(LeakyReLU(alpha=0.02))
...
def train(generator, discriminator, dataset_iterator, manager):
    ...
    for iteration, batch in enumerate(pbar):
        ...
        if iteration % 500 == 0:
            fid_ = fid_function(batch, generated)
            all_fids.append(fid_function(batch, generated))
            print('**** INCEPTION DISTANCE: %g ****' % fid_)

    return np.mean(all_fids)
